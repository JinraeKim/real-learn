...
class Aircraft3Dof(BaseSystem):
    ...
    def deriv(self, state, t, control, external):
        derivative_by_input, derivative_by_disturbance  = \
        self._raw_deriv(state, t, control, external) 
        return derivative_by_input + derivative_by_disturbance
    ...
    def _raw_deriv(self, state, t, control, external):
        ...
        # [0]: deriv by input, [1]: deriv by disturbance
        dxdt = [V*np.cos(gamma)*np.cos(psi), 0]
        dydt = [V*np.cos(gamma)*np.sin(psi), Wy]
        dzdt = [-V*np.sin(gamma), 0]

        dWydt = dWydz * sum(dzdt)

        dVdt = [
            (self.Tmax*T/self.m - self.term1*V**2*CD - self.g*np.sin(gamma)), 
            - dWydt*np.cos(gamma)*np.sin(psi)
        ]
        dgammadt = [
            (self.term1*V*CL*np.cos(phi) - self.g*np.cos(gamma)/V), 
            dWydt*np.sin(gamma)*np.sin(psi)/V
        ]
        dpsidt = [
            (self.term1*V/np.cos(gamma)*CL*np.sin(phi)),
            - dWydt*np.cos(psi)/V/np.cos(gamma)
        ]

        derivative_by_input = \
        np.array(
            [dxdt[0], dydt[0], dzdt[0], dVdt[0], dgammadt[0], dpsidt[0]]
        )
        derivative_by_disturbance = \
        np.array(
            [dxdt[1], dydt[1], dzdt[1], dVdt[1], dgammadt[1], dpsidt[1]]
        )
        return derivative_by_input, derivative_by_disturbance
